...
// directly. While attached, it must skip refreshPanesCmd, which was observed in
// the #598 trace adding to tmux-server contention. The synchronous mutations
// (mode, menu state) still run so
...
func TestSelectionChanged_SkipsRefreshWhileAttached(t *testing.T) {
	...
	// refreshPanesCmd.
	assert.Nil(t, cmd,
		"selectionChanged must return nil while attached: refreshPanesCmd must not "+
			"be queued behind the user's detach key (#598)")
}
