...
func (h helpTypeGeneral) toContentWidth(contentWidth int) string {
	...
	return renderHelpSections(header, []helpSection{
		{title: "Managing:", rows: []helpRow{
			...
			{helpKey(keys.KeyRestore), "Restore · sandboxes push before replacement or refuse"},
			{helpKey(keys.KeyLimitRetry), "Resume after a usage limit"},
			{navKeys, "Select a session"},
			{helpKey(keys.KeyEnter), "Type in the pane · all keys go to the agent"},
...
func (h helpTypeInteractive) toContent() string {
	content := lipgloss.JoinVertical(lipgloss.Left,
		...
		"",
		descStyle.Render("All keys, including tab, go to the agent."),
