...
func branchCommitWarning(worktreePath, branchName, recordedBaseSHA string) (string, bool) {
	...
	// branch, or refs/stash survive it. Enumerate ALL refs and exclude those that
	// survive cleanup, mirroring detachedHeadCommitWarning and
	// refSurvivesWorktreeCleanup so the branch half and the detached half agree on
	// durability for the same commit held by the same ref.
	//
	// Using --contains=<tip> would only find refs at or beyond the tip, missing
	// refs that point at earlier commits in base..branch. Enumerating all refs
	// unconditionally is correct: a ref that contains nothing in the range excludes
	// nothing from the git log output, so over-inclusion is harmless.
	...
	// the bug. Surviving refs are appended bare. A trailing `--` separates
	// revisions from path arguments, preventing ambiguity when a ref name also
	// matches an existing worktree path.
	...
	if refList, ferr := runKillGit(worktreePath, "for-each-ref", "--format=%(refname)"); ferr == nil {
		for _, ref := range strings.Split(strings.TrimSpace(string(refList)), "\n") {
			ref = strings.TrimSpace(ref)
			if ref == "" {
				continue
			}
			// The session branch itself does NOT survive cleanup
			// (refSurvivesWorktreeCleanup returns false for it), so it is
			// correctly NOT excluded — a commit held only by the session
