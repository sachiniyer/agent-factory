...
func TestGroupACopyNamesExistingActionsAndDefaults(t *testing.T) {
	choices := accountChoicesFrom(daemon.ListAccountsResponse{Defaults: map[string]string{"claude": "work"}, PoolRouting: true}, "claude")
	...
	// says so (#4404 review).
	ambient := accountChoicesFrom(daemon.ListAccountsResponse{PoolRouting: true}, "claude")
	...
	preRouter := accountChoicesFrom(daemon.ListAccountsResponse{}, "claude")
