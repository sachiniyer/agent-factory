...
- **Idempotence.** A *live* session named `issue-<n>` makes `af sessions create` fail (the script logs and continues), so re-running won't double-spawn across live sessions. An *archived* `issue-<n>` no longer blocks the create: when the archived session's title and branch can be reclaimed (the daemon moves an ordinary local archived session's branch aside; it refuses, for example, when the branch has been published and the archived worktree still holds it, or is held by another or multiple worktrees, the archived worktree is external or cannot be matched to its holder, or the destination archived branch already exists), it renames the archived row aside to the first free `issue-<n> (archived[ N])` and creates a new live `issue-<n>` — so a re-run *can* re-spawn an issue whose prior session was archived, but only if the cursor is not later than that issue's `created_at`. Deleting the state file re-initializes it to the current time (see the *Closed/triaged issues* caveat), so a reset or lost cursor does not by itself re-spawn them.
