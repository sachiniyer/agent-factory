...
- **Idempotence.** A *live* session named `issue-<n>` makes `af sessions create` fail (the script logs and continues), so re-running won't double-spawn across live sessions. An *archived* `issue-<n>` no longer blocks the create: when the archived session's title and branch can be reclaimed (the daemon refuses if the branch has been published or is otherwise held), it renames the archived row aside to the first free `issue-<n> (archived[ N])` and creates a new live `issue-<n>` — so a re-run *can* re-spawn an issue whose prior session was archived, but only if the cursor predates that issue's `created_at`. Deleting the state file re-initializes it to the current time (see the *Closed/triaged issues* caveat), so a reset or lost cursor does not by itself re-spawn them.
