...
var tasksShowCmd = &cobra.Command{
	...
	Long: "Show one task in the current project: its trigger, whether the running " +
		...
		"Overdue is derived, never stored: a cron task is overdue when its last run " +
		"precedes its most recent scheduled occurrence by more than one period (or " +
		"five minutes, whichever is larger). Watch tasks have no schedule and are " +
		"never overdue — their arming state is the signal.\n\n" +
