...
func init() {
	...
	sessionsCreateCmd.Flags().StringVar(&createAccountFlag, "account", "", "Credential account `name` to pin the agent to (register it with af accounts add; omit to route across the agent's logged-in accounts — preferring this project's default_accounts entry — and pass --account \"\" to pin the ambient identity)")
