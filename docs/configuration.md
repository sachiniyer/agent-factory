...
- **`ctrl+c` is a fixed hard exit,** not a reserved key: it always quits and is handled before the keymap, so it is never dispatched to an action and simply isn't rebindable. The structural keys above (`enter`, `tab`, etc.) are likewise fixed — they aren't rebindable *actions* in the first place, so there's nothing to point a `[keys]` entry at.
