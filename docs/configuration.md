...
- **An unloadable personal config fails closed.** If a registered project's personal config file exists but cannot be loaded when the daemon starts — unreadable permissions, a TOML syntax error, an invalid key — that file may hold the highest-precedence `enabled = false`, so the daemon will not start (or re-create) a root agent for that project, whatever the lower layers say. A root that is already running is left alone. This is the one root-agent state that deliberately does not self-heal on a later ensure pass: the daemon logs a warning naming the project at startup, and fixing the file takes effect on the next daemon start, like every other root-agent config change.
