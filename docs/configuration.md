...
| `require_token` | Whether the web/TCP listener requires the bearer token for **non-loopback** (network) peers (default `true`). Loopback peers are exempt by default (see `require_loopback_token`). Set `false` only on a trusted network to drop the token for network peers too (TLS still applies); the daemon logs a loud startup warning. Global-only. |
...
| `auto_yes`, `auto_update`, `require_token`, `listen_addr`, `tls_cert`, `tls_key`, `cors_allowed_origins`, `daemon_poll_interval`, `branch_prefix`, `worktree_root`, `detach_keys`, `log_max_size_mb`, `log_max_backups`, `update_channel`, `keys`, `root_agents`, `limit_auto_resume`, `limit_retry_interval` | Global only. Setting them in-repo is rejected with an error naming the key. The daemon network-surface keys (`require_token`, `listen_addr`, `tls_*`, `cors_allowed_origins`) are global-only so a cloned repo can never open a port, swap the cert, widen CORS, or disable auth. See [remote-tcp-auth.md](remote-tcp-auth.md). |
