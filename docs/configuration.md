...
on_archive_command = 'find node_modules -depth -delete'
...
| `preview_listen_addr` | Address for a **separate** plain-HTTP listener that will serve **web-tab previews** on their own origin, kept apart from `listen_addr` (#1856). Defaults to `""` (**disabled**) — no second port opens unless you set one. It exists so a previewed dev server can be framed **cross-origin** to the web UI, where it cannot reach the UI's bearer token; that is what will let absolute-path assets load. It accepts the same `host:port` grammar as `listen_addr`, and a bind conflict is logged and skipped, never fatal. It serves **web-tab previews only — never the daemon control API**, and today it serves nothing yet (the preview routes move onto it in a later step). A non-loopback value is warned about once at daemon start, like `listen_addr`. Global-only. |
