...
| `branch_prefix` | Prefix for the git branch each new session's worktree gets (defaults to `username/`). A registered project can override it in its [personal per-project config](#personal-per-project-config). A saved change applies to the next session created, with no restart. Docker and ssh sessions create their branch inside the sandbox, which uses the `af` config on that machine instead. |
...
Inspect exactly which layer wins, and why, with `af config get <key> --repo <path> --explain` (or `af config list --repo <path> --explain`): the trace shows the personal-project candidate alongside the others, marked won, shadowed, absent, or — for a key that cannot be overridden per project — disallowed. A running daemon reads this file each time it uses one of these keys, so no restart is needed: `default_program`, `program_overrides`, `default_accounts`, and `branch_prefix` apply to the next session created in the project, `on_archive_command` to the next archive, and `limit_account_candidates` to the next account switch. `[root_agent]` is the exception and waits for the next daemon start (see [Root agents](#root-agents-always-ensured)). A TUI that is already open keeps the project's `branch_prefix` it read at launch for the naming form's duplicate-title check, until you relaunch it or switch projects. The daemon still checks every create against the current value.
