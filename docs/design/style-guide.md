...
At widths up to 768px, keep project context and Sessions · Tasks · Config in the
app header, alongside the session drawer toggle. Fold install, Light · Dark ·
System and Disconnect into one … disclosure. Keep these controls available when
a session is focused.

Give the pane title, static Keyboard ownership label and Actions disclosure their
own row. Session titles truncate with …; preserve the complete name in accessible
text and the title tooltip. Tabs stay on one horizontally scrolling row. Controls
