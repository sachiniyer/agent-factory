...
- Left rail: **narrower** (#1090) — `clamp(22, 25 %·W, 36)` cols, full height. Inside the rail, the automations section is bottom-aligned: rule + up to ~4 rows, ceded to the tree as height tightens (2-row compact summary minimum, #1560). Status bar: 2 rows. Workspace: the entire remainder, **full vertical height** (#1090 — no bottom strip); N panes divide the width evenly with 1-col dividers.
