...
| `POST` | `/v1/ExplainConfig` | `key` | Explain one global config key's provenance: every source candidate with its value, location, and the reason it won, was shadowed, was absent, or is disallowed. |
