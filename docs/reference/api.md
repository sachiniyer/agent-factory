...
| `POST` | `/v1/DeleteProject` | `repo_path`, `repo_id` | Delete a project (a repo's session grouping): archive its live sessions (restorable; rows whose title claims the reserved root name are preserved but counted in unrestorable_count), tear down in-place ones, and drop its root_agents opt-in — the real git repo is untouched. |
