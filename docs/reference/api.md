...
| `POST` | `/v1/GetProjectConfig` | `project_path` | List one repository's effective config (every key `af config list --repo` shows, repo-scoped keys included) as the same manifest rows GetConfig returns — the project-scope read behind the web and TUI config surfaces. The path is a read-only selector: it never registers a project or writes identity state. |
