...
| `POST` | `/v1/SetPRInfo` | `title`, `repo_id`, `id`, `pr_info` | Compatibility route for older clients to record or clear GitHub PR info. New clients should call RefreshPRInfo with session identity only so discovery and projected fields stay daemon-owned. |
| `POST` | `/v1/RefreshPRInfo` | `title`, `repo_id`, `id` | Ask the daemon to refresh a session's GitHub PR projection. The request carries session identity only; discovery is cancellable and server-side debounced. Returns an error when gh is unavailable in the daemon environment. |
