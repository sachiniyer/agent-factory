...
| Path | Nested fields |
|------|---------------|
| `/v1/AddTask` | `task.id`, `task.name`, `task.prompt`, `task.cron_expr`, `task.watch_cmd`, `task.target_session`, `task.max_concurrent_runs`, `task.on_complete`, `task.project_path`, `task.repo_id`, `task.program`, `task.enabled`, `task.created_at`, `task.last_run_at`, `task.last_run_status`, `task.last_run_session_id`, `task.audit`, `task.overdue`, `task.missed_occurrences`, `task.missed_occurrences_capped`, `task.unschedulable`, `task.unschedulable_reason`, `task.unassessable`, `task.next_run_at`, `task.arming`, `task.ordinal`, `task.store_generation` |
