...
| `POST` | `/v1/ListAccounts` | `agent` | List the agent accounts registered in the daemon host's agent-factory home, each with its logged-in state (read from the presence of the agent's own credential file, never its contents), plus the agents an account can be registered for. |
