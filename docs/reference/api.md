...
| `POST` | `/v1/QuotaReport` | — | Report usage-limit status for each agent CLI on this host: af's own observations (parked sessions, when the wall was recorded) kept apart from the provider's entitlement answer, which af has no API to read. |
