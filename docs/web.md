...
- **Delete session** permanently tears down the session and removes its record. It removes
  ...
  The daemon-managed root is the exception: deleting it stops scheduled and
  watch-task delivery until it self-heals (usually about two minutes) or the daemon
  restarts — disable it in [configuration](configuration.md#root-agents-always-ensured)
  to keep it down.
