...
- **Events over the cap are queued, never dropped.** They land in the same durable per-task queue as a failed delivery (above), replay **in emission order**, and survive daemon restarts.
