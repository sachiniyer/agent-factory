...
| `generation_id` | Store-owned identity for this exact task incarnation. Minted anew on every add, including reuse of the same `id`; sessions retain it so a surviving session from a removed task cannot write status onto its replacement. A row written before this field existed is given a `legacy-`-prefixed generation the first time the daemon loads it, recorded in `audit` with the `daemon-upgrade` actor; that row keeps its queued watch events and its `on_complete` setting. Read-only to clients |
