...
| `unassessable` | **Derived at read time, never stored.** No verdict could be reached: the record has neither a last run nor a creation time to measure from. Unknown, not healthy — only a hand-edited row can reach it, since every create is stamped |
