...
| `last_run_session_id` / `last_run_sequence` | Scheduler-owned identity and ordering proof for session-per-run deliveries. Outcomes match the stable session id; delayed start writers compare the daemon-assigned monotonic sequence. Neither treats a wall-clock timestamp as identity or order. Read-only to clients. A target-session delivery clears the id but retains the sequence as its session-run high-water mark; both are absent when there is no session-per-run history or the row predates these fields |
