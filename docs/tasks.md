...
| `generation_id` | Store-owned identity for this exact task incarnation. Minted anew on every add, including reuse of the same `id`; sessions retain it so a surviving session from a removed task cannot write status onto its replacement. Read-only to clients; absent on pre-field rows |
