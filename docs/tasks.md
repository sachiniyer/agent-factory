...
| `last_run_revision` | Scheduler-owned revision incremented by every last-run mutation. Session creation captures it before provisioning and publishes only if it is unchanged, so a later watcher `stopped` / `errored` observation cannot be overwritten by delayed startup. Read-only to clients; absent before the first status write |
