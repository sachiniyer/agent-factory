...
| `last_run_session_id` | Scheduler-owned identity of the session-per-run delivery represented by the current last-run fields. Outcomes use this stable token rather than a wall-clock timestamp, so concurrent runs cannot publish an outcome for one another. Read-only to clients; omitted for target-session deliveries and rows written by older versions |
