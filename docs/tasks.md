...
Lateness is measured from the **latest** of the last run, the last time the task was *enabled*, and (for a task that has never run) its creation. A re-enable is a fresh start, exactly as a first run is: without that rule a task deliberately paused and later switched back on would report every occurrence it missed while intentionally off — #3623's own tasks, disabled 2026-08-14 and re-enabled 2026-09-01, would have read `overdue · missed 432` from the moment they came back until their next fire. The audit trail below is where that enable time comes from, so it is bounded too: an enable old enough to have fallen out of the window leaves the reference at the last run, which can only make the verdict more eager, never hide a task that has genuinely stopped.
