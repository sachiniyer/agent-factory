...
| `on_complete` | What happens to a session this task spawned once its run finishes: `keep` (the default — leave it in place), `archive`, or `kill`. Rejected on a `target_session` task, whose session exists to be reused. A session that was already running when you upgraded to a version with `generation_id` is always kept; runs started after the upgrade follow this setting (see [What happens to a run's session](#what-happens-to-a-runs-session)) |
...
| `generation_id` | Store-owned identity for this exact task incarnation. Minted anew on every add, including reuse of the same `id`; sessions retain it so a surviving session from a removed task cannot write status onto its replacement. A row without one (written before this field existed, or added to `tasks.json` by hand) is given a `legacy-`-prefixed generation the first time the daemon loads it or starts a run for it, recorded in `audit` with the `daemon-upgrade` actor; that row keeps its queued watch events and its `on_complete` setting. Read-only to clients |
| `last_run_at` / `last_run_status` | Set by the daemon: `started` (a session-per-run delivery created its session), `sent` (the prompt was delivered into a shared target, including one auto-created for it), `parked: usage limit` (a created or targeted session is waiting at a plan usage-limit wall, not failed — see [usage-limits.md](usage-limits.md#task-runs-park-dont-fail)), `interrupted: agent runtime lost` (restore or daemon startup replaced the runtime that received the prompt without replaying it, so the session was retained and `on_complete` was skipped; a durable session marker retries this task status across daemon restarts), and for watch tasks `stopped`, `errored`, or `dropped: event rate limit exceeded` (see below) |
| `last_run_session_id` / `last_run_sequence` | Scheduler-owned identity and ordering proof for session-per-run deliveries. Outcomes match the stable session id, and an earlier run's outcome is not recorded while a later run of the same task exists; delayed start writers compare the daemon-assigned monotonic sequence. Neither treats a wall-clock timestamp as identity or order. Read-only to clients. A target-session delivery clears the id but retains the sequence as its session-run high-water mark; both are absent when there is no session-per-run history or the row predates these fields |
| `last_run_revision` | Scheduler-owned barrier incremented by every task-wide, non-session status write. Session creation captures it before provisioning and publishes only if it is unchanged, so a later watcher `stopped` / `errored` observation cannot be overwritten by delayed startup. Concurrent session-run publications instead use their generation, stable session id, and monotonic sequence, so an older run cannot block a newer admitted run. Read-only to clients; absent before the first task-wide status write |
| `dropped_events` | Cumulative source events this watch task lost without delivering or queueing them: events past `watcher_events_per_minute`, and undelivered events the durable replay queue could not accept (no queue available, a failed append, or a stop while the queue was unreadable). Every one of them records `last_run_status` as `dropped: event rate limit exceeded`, whatever the cause. Daemon-owned and not editable. It does not count the separately bounded durable queue's overflow or expiry |
...
| `unschedulable` | **Derived at read time, never stored.** The scheduler cannot derive a next run from this task's cron expression — it does not parse, or nothing matches within its search horizon. Such a task is not overdue (nothing was ever due) and is not healthy either |
...
- **Rate limit**: at most `watcher_events_per_minute` events per minute per task (default: 10). Excess events are dropped — never silently: the cumulative count is stored as `dropped_events`, returned by `af tasks list`, and logged. A dropped event records `last_run_status` as `dropped: event rate limit exceeded` rather than leaving the loss indistinguishable from `sent`; later successful deliveries may update that status, but the cumulative count remains. Rate-dropped events are not queued for replay (the limit is protective policy against a chatty script, not an outage signal). This is a limit on the event *rate*, and is independent of `max_concurrent_runs`, which bounds in-flight *sessions* and queues rather than drops — the two are never the binding constraint at the same time (see [Limiting concurrent sessions](#limiting-concurrent-sessions)).
...
- **Delivery failures are queued and replayed**: an event whose delivery fails — the target session unreachable, e.g. during a tmux outage — is appended to a durable per-task queue under `~/.agent-factory/events/` instead of dropped, and replayed **in emission order, before newer live events**, once deliveries succeed again (under the same configured rate window). The backlog survives daemon restarts and reloads. Semantics and bounds:
  ...
  - **Usage-limit park**: when `target_session` is already `[limit]`, the event is retained for replay instead of typed into the pane. This known, bounded recovery wait is exempt from the 72h expiry and oldest-event overflow policy. At the same 500-event / 256KB threshold, the daemon backpressures the watch command's stdout until replay makes room, keeping AF's disk use bounded without dropping distinct events. The on-disk park marker and backlog both survive daemon restarts; as with every watcher restart, scripts should retain their own source cursor for output not yet read from the pipe.
  - **Disabled vs deleted**: a disabled task keeps its backlog and replays it on re-enable; a deleted task's queue is removed. A backlog queued before the task had a `generation_id` stays with the task when the daemon gives it one.
...
- **A slot frees when the session goes idle**, or when it is archived or killed. It does *not* wait for you to archive the session: a cap that only freed on archive would stall the task until a human intervened, and the backlog would age out. It also does not wait for `post_worktree_commands` to finish, so a hung hook cannot wedge the task permanently.
...
- **A run interrupted mid-flight keeps its slot** while the daemon is still trying to restore it. Freeing it sooner would let the task admit replacements and then exceed its cap once the originals came back. The restore loop retries with exponential backoff and gives up after a bounded number of failures, releasing the slot at that terminal give-up; `af sessions kill` is the manual off-ramp to free it sooner.
...
- **A session started before your task had a `generation_id` is kept.** Nothing on such
  a session proves it belongs to the task that now has its `id`. It could be from a
  task you removed whose `id` was later reused. So when you upgrade to a version with
  `generation_id`, sessions that were already running stay in place when their runs
  finish. Runs the task starts after the upgrade carry the generation the daemon gives
  the task and follow `on_complete`. If such a pre-upgrade session later loses its agent
  runtime, the interruption is recorded on the session, but the task's
  `last_run_status` may not show it.
