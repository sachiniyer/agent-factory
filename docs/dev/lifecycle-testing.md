...
| **no version skew** (the #1921 check) | `lc_assert_eq "" ""` — if the daemon's version came back empty (it never answered *and* the `/proc/<pid>/exe` fallback were unreadable) *and* the client version came back empty, the most important assertion here would compare nothing to nothing and report "no skew" | both operands must be non-empty or it fails |
