...
For scripting `af` — from a shell, a CI job, or another agent. After this page you
will know how a command picks its project, which groups emit JSON, and what each
verb does.
...
| Flag | Description |
|------|-------------|
| `-p`, `--program` | Agent to run in new sessions, one of `claude`, `codex`, `aider`, `gemini`, `amp`, `opencode`, `devin`. To pass custom paths or flags, use `program_overrides` in the config instead — see [configuration.md](configuration.md#choosing-the-agent). |
...
- **Everything else** — `kill`, `archive`, `restore`, `retry-limit`, `send-prompt`, tab create/delete/rename/reorder — reaches the *local* daemon regardless of `--daemon-url`, so it stays scoped to the current directory.
...
af sessions tab-rename <title> --name <tab> --new-name <n>   # rename a tab's handle
af sessions tab-reorder <title> --name <tab> --index <n>     # move a tab in the order
...
af sessions tabs rename <title> --name <tab> --new-name <n>  # alias for tab-rename
af sessions tabs reorder <title> --name <tab> --index <n>    # alias for tab-reorder
af sessions preview <title>                               # snapshot the session's pane
af sessions watch <title>                                 # block until the agent is idle (fleet form with no title)
...
- `tab-rename`: renames a tab's handle — `--name` (the tab to rename) and `--new-name` are both required. The new name is sanitized and collision-suffixed exactly like `tab-create --name`, with one difference: a `--new-name` that sanitizes to nothing is an error rather than falling back to a default name. The resolved name prints as `{"name": "..."}`. Only web, process, and VS Code tabs can be renamed — those are the kinds that display their name; the agent tab always shows `Agent` and shell tabs always show `Terminal`, so renaming one is refused. The rename persists across a daemon/`af` restart and does not disturb the tab's running process. On remote sessions only admitted metadata-only web tabs can be renamed; archived sessions must be restored first.
- `tab-reorder`: moves a tab — `--name` and `--index` are both required. `--index` is the destination slot, 0-based, counting the tab bar left to right, and read as the tab's *final* position. Slot 0 is reserved for the agent tab: it can't be moved, and no tab can move in front of it. The new order persists across a daemon/`af` restart and disturbs no tab's process. On remote sessions only admitted metadata-only web tabs can be reordered; archived sessions must be restored first. Prints `{"name": "...", "index": <n>}`.
- `tabs {create,delete,rename,reorder}`: additive noun-subcommand aliases — `af sessions tabs create` == `af sessions tab-create`, and likewise for `delete`, `rename`, and `reorder` (same flags and output). The hyphen verbs are kept for existing scripts; nothing is renamed. There is no `tabs list` — list a session's tabs via `af sessions get`.
- `watch`: blocks until a session's agent is idle and awaiting input, then exits 0 — idle does not mean the work succeeded. It polls the daemon's snapshot every `--interval` (default 2s) and exits non-zero if the session reaches a state it cannot leave on its own (lost, dead, or archived), if it disappears, or if `--timeout` elapses first (default 30m). With no `<title>` (or `--all`) it watches every session in scope and returns on the first stop-state change or disappearance, printing which session and why; that fleet form is edge-triggered, so a session already idle when you call does not fire unless `--include-current` is passed to report the starting snapshot (archived sessions are omitted from it — inert by construction — while a session archived *while* you watch is still reported). Unlike the single-title form, a reported fleet event still exits 0 — even on `lost`, `dead`, `archived`, `killed`, or `gone` — so automation must branch on each event's `reason` rather than the exit status; only a polling error or `--timeout` is non-zero. With `--json`, the single-title form emits the final session record; the fleet form emits `{"events": [...]}`, each event naming the session, its stop reason, and — except on `gone` — the nested session record.
...
For TUI appearance, use `af config set appearance light`, `dark`, or `system`
(default); `af config get appearance` reads the choice. It takes effect at the
next TUI launch on that machine. The CLI has no persistent renderer, and the
browser's header preference is independent. System uses terminal background
with a dark fallback in the TUI and OS appearance in the browser.

The CLI rejects `theme` and all `theme.*` CLI keys
explicitly, directing users to `appearance`. On read, legacy `theme` Light/Dark
values migrate to matching appearance; auto/system/Nord/Zenburn/custom tables
migrate to System, with any existing valid appearance taking precedence. The
migration logs the old key and replacement value and persists once,
idempotently. No separate `af config migrate` invocation is needed for this
retirement. Read-only diagnostics (`LoadConfigReadOnly` and `af config validate`)
preserve their no-write contract and do not persist migration changes. See
[Appearance migration](configuration.md#appearance-and-legacy-theme-migration).
Launching the TUI reads appearance locally and must never activate pending
listener/auth edits; explicit apply-on-save remains available.
...
finishing — advisory warnings do not.
