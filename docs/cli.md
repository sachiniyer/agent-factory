...
Every `af config` verb is **local-only**: each reads or writes the config on the machine it runs on, so `--daemon-url`/`AF_DAEMON_URL` is refused rather than silently ignored (#3661, #3679). Those are persistent root flags and therefore appear in every subcommand's `--help`; accepting one here and dropping it would hand you a confident verdict about the wrong host — or, on `set`, `unset`, and `migrate`, a success line naming a local path for a change you believe you made remotely. Run the command on the daemon host to act on that host. Administering a remote daemon's config through the CLI — routing global `set`/`unset` to the daemon's admission-gated write — is tracked separately as a feature (#3679).
