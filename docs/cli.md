...
`get`, `list`, `validate`, and `migrate` are **local-only**: they read (and, for `migrate`, rewrite) the config on the machine they run on, so `--daemon-url`/`AF_DAEMON_URL` is refused rather than silently ignored (#3661). Those flags are persistent root flags and therefore appear in every subcommand's `--help`; accepting one here and dropping it would hand you a confident verdict — or a believed-in migration — for the wrong host. Run the command on the daemon host to ask about that host. `set` and `unset` are unchanged by this.
