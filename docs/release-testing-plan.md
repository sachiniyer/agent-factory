...
| Remote hooks | Integration test runs the full provision-and-expose round-trip (#1592 Phase 4 PR7): `launch_cmd` starts a real `af agent-server`, the daemon drives it over `http://`/`ws://`, and `delete_cmd` reaps it on kill. Covers bad JSON, command failures, and timeout-driven teardowns. | Configure `examples/remote-hooks` or a repo-local mock hook set, create a remote session, preview/attach, and kill it. |
...
- Real Claude/Aider/Codex/Gemini/Amp/opencode/devin interactive behavior is not fully automated;
