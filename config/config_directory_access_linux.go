...
func checkConfigDirectoryAccess(path string) error {
	...
	if os.Getuid() == os.Geteuid() && os.Getgid() == os.Getegid() {
		...
		// real-ID fallback when neither set contains any capabilities.
		hdr := unix.CapUserHeader{Version: unix.LINUX_CAPABILITY_VERSION_3}
		data := [2]unix.CapUserData{}
		if err := configDirectoryCapget(&hdr, &data[0]); err == nil &&
			data[0].Effective|data[1].Effective|data[0].Permitted|data[1].Permitted == 0 {
			flags = 0
		}
	}
	// For differing credentials/capabilities keep x/sys's effective-access
