...
// are the five registered runtimes; validation of the VALUE lives at
...
//
// The five canonical `backend` values. `local` (or empty) is the default —
// today's in-process tmux+worktree runtime, unchanged. `hook` is the existing
// remote-hook backend. `docker`, `ssh`, and `sandbox` are the off-box
// sandboxed runtimes.
const (
	BackendLocal   = "local"
