...
// This file holds the FOLLOWED-LINK lock: the machinery that resolves a symlink
// once, pins the directory it resolved into, and hands the body a handle that
// can only touch that one resolution (#3688, #3697).
//
// It was split out of filelock.go, which had grown past the file-length limit
// (#3736). The cut is by subject rather than by size: everything here exists
// because a followed write must agree with the lock that covers it, while
// filelock.go keeps the path-based locks and writers that never follow
// anything. Nothing moved changed.
