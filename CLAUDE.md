...
- `main.go` — entry point; builds the `af` root command and launches the TUI
- `commands/` — Cobra commands (`config`, `daemon`, `accounts`, `doctor`, `reset`, `upgrade`, ...)
...
- `api/` — the `af sessions`/`tasks`/`projects`/`api` CLI verbs, talking to the daemon over HTTP/net-rpc
...
- `cmd/` — thin `exec.Cmd` wrapper (process execution seam for tests, not CLI commands)
- `log/` — logging
