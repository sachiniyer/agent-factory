...
Differing blob SHAs between the PR's files and the content head rule carry out
immediately (cheap negative) without needing the tree proof. The full check also
reads the merge base and both parent trees, derives the only path-level
three-way result, and requires the merge commit's tree to match it exactly — a
truncated, malformed, same-path-conflicting or mismatched tree refuses carry.
