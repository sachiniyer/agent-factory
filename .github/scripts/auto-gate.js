...
//
// Every other statement of it — this file's own function comment, the gate-pr
// skill, and .github/auto-gate.md — quotes this constant, and a test fails if
// any of them stops saying it. The rule was written out four times before, twice
// in this file alone, and those two copies had already drifted apart while every
// test stayed green. The copy furthest from the code is the one a maintainer
// reads before hand-merging, so it is the one that must not rot.
//
// Quote it; do not paraphrase. Reflowing is free — the check normalises comment
// markers, markdown emphasis and line wrapping — but changing what it says is
// not.
const CODEX_LIMIT_RULE =
  "A Codex usage-limit message counts as a review outage unless its scope clause names a job " +
  "this repository has OBSERVED naming something other than review. An unobserved phrasing " +
  "counts, because a false block during a real outage has no exit while a false degrade is a " +
...
//
// Implements CODEX_LIMIT_RULE — see the constant for the rule and why it points
// this way. The history is there too; it is not repeated here, because the copy
// that used to sit in this comment had already drifted from the one above it.
//
