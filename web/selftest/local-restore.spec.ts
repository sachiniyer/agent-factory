...
for (const [backend, outcome] of [["local", "refused"], ["docker", "refused"], ["local", "dismissed"], ["local", "success"], ["local", "warning"], ["local", "uncertain"]]) {
  test(`4017: ${backend} restore consent and ${outcome}`, async ({ page, request }, info) => {
    ...
    if (outcome === "dismissed") await page.keyboard.press("Escape");
    release();
    if (outcome === "refused") {
      ...
      if (outcome === "success") await expect(page.locator(".af-row-selected")).not.toHaveClass(/af-row-archived/);
