...
for (const kind of [TabKind.Shell, TabKind.Process, TabKind.VSCode, TabKind.Web]) {
  test(`tab deletion asks for the exact target, kind ${kind}`, async ({ page, request }, info) => {
    ...
    const target = { id: "consent-tab", name: kind === TabKind.Shell ? "shell-2" : kind === TabKind.Web ? "web" : kind === TabKind.VSCode ? "vscode" : "exact build logs", kind };
    ...
    const label = await page.locator(".af-tab-label").nth(1).innerText();
    ...
    await expect(modal.locator(".af-modal-title")).toHaveText(`Delete tab “${label}” from session “${session.title}”?`);
    ...
    await expect(page.locator(".af-tab-close").last()).toHaveAttribute("title", `Delete tab “${label}”`);
