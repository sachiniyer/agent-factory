 ...
 */
async function touchTap(cdp: CDPSession, x: number, y: number, jitterPx = 3): Promise<void> {
  await cdp.send("Input.dispatchTouchEvent", { type: "touchStart", touchPoints: [{ x, y }] });
  await cdp.send("Input.dispatchTouchEvent", { type: "touchMove", touchPoints: [{ x, y: y + jitterPx }] });
  await cdp.send("Input.dispatchTouchEvent", { type: "touchEnd", touchPoints: [] });
}
...
test("#2682 mobile: one finger scrolls the terminal, and keeps doing so under application mouse mode", REAL_FIXTURE, async ({
  ...
  try {
    ...
    expect(
      ...
      "a tap's jitter is not a scroll — it must leave the view where it was",
    ).toBe(beforeTap);
