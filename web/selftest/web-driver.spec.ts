...
// The mirror-path / misroute session (#1806/#1810/#1811): tabs [agent, lower, vite,
// after], where "vite" targets a SUBDIRECTORY document with sibling, parent-relative
// and absolute-path assets, and "lower"/"after" both point at the plain preview
// server. See web-selftest-entry.sh for why that layout is load-bearing.
...
// The #1810 misroute session — tabs [agent, lower, mis, after] — visited by exactly
// one test, on purpose (see web-selftest-entry.sh: re-selecting a session desyncs the
// store's activeTab from the pane, which would yank the pane to Agent on any close).
