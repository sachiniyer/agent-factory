...
test("lifecycle confirmation copy fits a 120-character reading budget", () => {
  ...
  for (const body of bodies) assert.ok([...body].length <= 120, `${[...body].length} characters: ${body}`);
  assert.match(bodies[0], /permanent|undo/i);
  assert.match(bodies[0], /owned by af/);
