...
// project summaries (counts + working glance), scoping a session list to one project,
// picking a sensible default, and reconciling the selection when the session set
...
test("projectSummaries: groups every session by repo root, sorted by path, with counts", () => {
...
test("projectSummaries: an archived-only repo still lists as a project (glance = no sessions)", () => {
  const s = projectSummaries([archived("gone", "/repos/shelved")]);
  assert.equal(s.length, 1, "the repo is still a project — folded delete leaves it navigable");
  assert.equal(s[0]?.liveCount, 0);
  assert.equal(projectMeta(s[0]!), "no sessions yet");
});

test("projectSummaries: sessions without a repo_path are skipped", () => {
  assert.equal(projectSummaries([sess("orphan", { id: "1" })]).length, 0);
});
...
test("defaultProject: the most-recently-active (latest created_at) live session's repo", () => {
  ...
  assert.equal(defaultProject([]), null, "no sessions → no default");
});

test("defaultProject: falls back to a newest archived repo only when nothing is live", () => {
  const all = [archived("a", "/repos/only", "2026-02-01T00:00:00Z")];
  assert.equal(defaultProject(all), "/repos/only");
});
