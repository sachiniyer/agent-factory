...
func unwrapIonice(words []*syntax.Word, names map[string]struct{}, memo operandTailMemo) ([]*syntax.Word, bool) {
	...
	for len(words) > 0 {
		...
		case option == "-t" || option == "--ignore":
			// -t/--ignore take no value and do not end option parsing, so the
			// child is whatever later word is not an ionice option. A shadowed
			// `./ionice` with `shift; exec "$@"` can still land on any literal
			// suffix of that child tail, so the every-suffix walk runs at the
			// child-return branches (`--` and `default`) after all options —
			// including the #4460/#4532 dynamic `-c"$CLASS"` token — are consumed.
			// Walking here would judge future option words (a non-literal
			// `-c"$CLASS"` is a legitimate admitted token, not an exec boundary)
		...
		case option == "-c" || option == "-n" || ioniceClassValueLongOption(option):
			...
			// No every-suffix walk here: the real child begins only at a later
			// word, after any further ionice options (including the
			// #4460/#4532 dynamic `-c"$CLASS"` token) are consumed. Walking the
			// tail now would judge those future option words — a non-literal
			// `-c"$CLASS"` is a legitimately admitted option token, not a
			// possible exec boundary — and fail closed on it. The child tail is
			// walked at the child-return branches (`--` and `default`).
			words = words[2:]
