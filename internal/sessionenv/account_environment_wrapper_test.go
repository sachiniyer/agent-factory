...
func TestValidateAccountEnvironmentCommand_WrapperGuardStaysNarrow(t *testing.T) {
	for _, command := range []string{
		...
		// assignment overrides no running agent. Spelled with wrappers this guard
		// still treats as unmodeled: strace carries its own operand-arity model
		// (#4287), which proves the child boundary and therefore hands the child to
		// the modeled path, exactly as `nohup env CODEX_HOME=/other` already is.
		"perf env CODEX_HOME=/other",
		"valgrind env CODEX_HOME=/other",
		// A dynamic tail word could expand to `env`, but with no command word
		// after the assignment the result is still print mode.
		"perf $W CODEX_HOME=/other",
...
func TestCommandMutatesAccountEnvironment_UnmodeledWrapperAssignment(t *testing.T) {
	...
	cases := []struct {
		...
		// against, so noun-uses stay allowed under a wrapper that is still
		// unmodeled and therefore still scanned as an opaque tail.
		{"perf sh", false},
		{"echo sh", false},
		{"man sh", false},
		{"perf -p 1234 sh", false},
		// strace is no longer one of those: its operand-arity model (#4287)
		// resolves the child boundary, so the shell arrives at command position
		// and takes the modeled path's verdict — the same one the modeled
		// wrappers and the bare form already produce. The controls below pin that
		// this is consistency with the modeled path, not a strace-specific rule.
		{"strace sh", true},
		{"strace -p 1234 sh", true},
		{"nohup sh", true},
		{"sh", true},
		// Same reason for env's print mode: a proven child boundary means the
		// modeled env arm judges it, as it already does under nohup and bare.
		{"strace env CODEX_HOME=/other", true},
		{"strace $W CODEX_HOME=/other", true},
		{"nohup env CODEX_HOME=/other", true},
