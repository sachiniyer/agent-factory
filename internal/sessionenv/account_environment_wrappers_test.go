...
func TestAccountCommandWrapperSpecsDoNotOverlapOptionKinds(t *testing.T) {
	for name, spec := range accountCommandWrapperSpecs {
		...
		for kind, options := range map[string]string{
			...
			"optional value": spec.shortOptionalAttached, "terminal": spec.shortTerminal,
			"environment": spec.shortEnvironment, "executable output": spec.shortExecutableOutput,
		...
		for kind, options := range map[string]map[string]struct{}{
			"no value": spec.longNoValue, "value": spec.longValue,
			"optional value": spec.longOptionalAttached, "terminal": spec.longTerminal,
			"environment": spec.longEnvironment, "executable output": spec.longExecutableOutput,
