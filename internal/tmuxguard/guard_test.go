...
func TestDenialReason(t *testing.T) {
	tests := []struct {
		...
		{name: "git grep abbreviated pager dispatcher", command: `git grep --open='tmux kill-server' pattern`, wantReason: unknownShellReason},
		{name: "git grep shortest pager abbreviation", command: `git grep --op='tmux kill-server' pattern`, wantReason: unknownShellReason},
		...
		{name: "file abbreviated external decompressor", command: `file --uncompress-n archive.gz`, wantReason: unknownShellReason},
		...
		{name: "literal go source path", command: `go run ./cmd/tool`},
		{name: "literal make target", command: `make test-container`},
		...
		{name: "make immediate variable overrides", command: `make 'SHELL:=tmux' '.SHELLFLAGS:=kill-server' test`, wantReason: unknownShellReason},
		...
		{name: "literal Python script path", command: `python3 ./script.py "$argument"`},
		{name: "literal Python script path after terminator", command: `python3 -- ./script.py "$argument"`},
		...
		// Exact-head Codex review findings against 2242a8bf. These subtests
		// were added and run fail-first before the corresponding fixes.
		{name: "git abbreviated external diff dispatcher", command: `git diff --ext`, wantReason: unknownShellReason},
		{name: "git abbreviated textconv dispatcher", command: `git diff --tex`, wantReason: unknownShellReason},
		{name: "git abbreviated merge strategy dispatcher", command: `git merge --str=ours branch`, wantReason: unknownShellReason},
		{name: "docker dynamic global option operand", command: `docker --host "$(printf ssh://host)" ps`, wantReason: unknownShellReason},
		{name: "docker dynamic short global option operand", command: `docker -H "$host" ps`, wantReason: unknownShellReason},
		{name: "docker literal global option operand", command: `docker --host unix:///tmp/docker.sock ps`},
		{name: "GitHub subcommand browser flag", command: `gh pr view 1 --web`, wantReason: unknownShellReason},
		{name: "GitHub subcommand short browser flag", command: `gh pr view 1 -w`, wantReason: unknownShellReason},
		{name: "GitHub create browser flag", command: `gh pr create --web`, wantReason: unknownShellReason},
		{name: "GitHub dynamic data after terminator", command: `gh pr view -- "$pr"`},
		{name: "sed sandbox after expression", command: `sed -e 'e tmux kill-server' --sandbox /etc/passwd`, wantReason: unknownShellReason},
		{name: "sed sandbox before expression", command: `sed -n --sandbox -e 'e tmux kill-server' /etc/passwd`},
		{name: "go double dash vet tool", command: `go vet --vettool=/tmp/evil ./...`, wantReason: unknownShellReason},
		{name: "go double dash tool executor", command: `go test --toolexec=/tmp/evil ./...`, wantReason: unknownShellReason},
		{name: "go ordinary double dash flag", command: `go test --race ./internal/tmuxguard`},
		{name: "agent browser is a dispatcher", command: `agent-browser open https://example.com`, wantReason: unknownShellReason},

		// Exact-head Codex review findings against 4d8b166e. These subtests
		// were added and run fail-first before the corresponding fixes.
		{name: "subshell cwd reaches nested make", command: `(cd /tmp; make test)`, wantReason: unknownShellReason},
		{name: "command substitution cwd reaches nested make", command: `echo "$(cd /tmp; make test)"`, wantReason: unknownShellReason},
		{name: "process substitution cwd reaches nested make", command: `cat <(cd /tmp; make test)`, wantReason: unknownShellReason},
		{name: "parent cwd reaches nested make", command: `cd /tmp; (make test)`, wantReason: unknownShellReason},
		{name: "pipeline branch cwd does not reach sibling", command: `cd /tmp | make test`},
		{name: "pipeline block cwd reaches nested make", command: `{ cd /tmp; make test; } | cat`, wantReason: unknownShellReason},
		{name: "process substitution cwd does not escape", command: `cat <(cd /tmp); make test`},
		{name: "background block cwd reaches nested make", command: `{ cd /tmp; make test; } &`, wantReason: unknownShellReason},
		{name: "parent assignment reaches nested git", command: `FOO=bar; (git status)`, wantReason: unknownShellReason},
		{name: "subshell assignment reaches nested git", command: `(FOO=bar; git status)`, wantReason: unknownShellReason},
		{name: "subshell assignment does not escape", command: `(FOO=bar); git status`},
		{name: "prior assignment reaches data command", command: `FOO=bar; printf safe`, wantReason: unknownShellReason},
		{name: "middle assignment reaches later git", command: `printf safe; FOO=bar; git status`, wantReason: unknownShellReason},
		{name: "trailing assignment does not flow backward", command: `printf safe; FOO=bar`},
		{name: "trailing export does not flow backward", command: `printf safe; export AGENT_FACTORY_HOME=/tmp`},
		{name: "GitHub auth login opens browser", command: `gh auth login`, wantReason: unknownShellReason},
		{name: "GitHub auth refresh opens browser", command: `gh auth refresh`, wantReason: unknownShellReason},
		{name: "Docker SSH host dispatch", command: `docker --host ssh://example.com ps`, wantReason: unknownShellReason},
		{name: "Docker short SSH host dispatch", command: `docker -H ssh://example.com ps`, wantReason: unknownShellReason},
		...
		{name: "cd reaches cwd-sensitive makefile", command: `cd /tmp; make test`, wantReason: unknownShellReason},
		{name: "command cd reaches cwd-sensitive makefile", command: `command cd /tmp; make test`, wantReason: unknownShellReason},
		{name: "cd before data command", command: `cd /tmp; printf safe`},
		{name: "subshell cd before make", command: `(cd /tmp); make test`},
		{name: "command substitution cd before git", command: `echo "$(cd /tmp)"; git status`},
		{name: "pipeline cd before make", command: `cd /tmp | cat; make test`},
		{name: "background cd before make", command: `cd /tmp & make test`},
		{name: "block cd reaches make", command: `{ cd /tmp; }; make test`, wantReason: unknownShellReason},
