...
func TestDefinitiveNonGitRepository_RecognizesBothOutsideRepositoryVariants(t *testing.T) {
	env := append(repositoryPathEnvironment(os.Environ()), "GIT_TERMINAL_PROMPT=0", "LC_ALL=C")
...
func TestDefinitiveNonGitRepository_FiltersUnrelatedAndGitDirErrors(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		"git-dir-set-ceiling": {
			...
			env:    append(append(repositoryPathEnvironment(os.Environ()), "GIT_TERMINAL_PROMPT=0", "LC_ALL=C"), "GIT_DIR=/tmp/unrelated"),
		},
