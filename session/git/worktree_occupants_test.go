...
func TestWorktreeOccupants_FindsAMarkerlessProcessByItsWorkingDirectory(t *testing.T) {
	worktree := t.TempDir()
	pid := parkedProcessIn(t, worktree)

	occupants, err := WorktreeOccupants(worktree)
...
func TestWorktreeOccupants_FindsAProcessInASubdirectory(t *testing.T) {
	worktree := t.TempDir()
...
func TestWorktreeOccupants_IgnoresAProcessOutsideTheWorktree(t *testing.T) {
	worktree := t.TempDir()
	elsewhere := t.TempDir()
...
func TestWorktreeOccupants_DoesNotMatchASiblingSharingAPathPrefix(t *testing.T) {
	parent := t.TempDir()
...
func TestWorktreeOccupants_RefusesAnEmptyPath(t *testing.T) {
	occupants, err := WorktreeOccupants("")
	require.Error(t, err)
...
func TestWorktreeOccupants_MissingWorktreeHasNoOccupants(t *testing.T) {
	occupants, err := WorktreeOccupants(filepath.Join(t.TempDir(), "never-created"))
	require.NoError(t, err)
