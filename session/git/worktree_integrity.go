...
// the daemon status loop and doctor, so a stalled checkout must not stall either
// caller indefinitely.
...
func InspectWorktreeIntegrity(worktreePath string) (WorktreeIntegrity, error) {
	...
	status, err := runIntegrityGit(worktreePath, "status", "--porcelain=v2", "--branch", "--untracked-files=no")
	if err != nil {
	...
	reflog, err := runIntegrityGit(worktreePath, "log", "-g", "-1", "--format=%H", "HEAD")
	if err != nil {
		return WorktreeIntegrity{}, err
	}
	...
	result.HeadMovedWithoutReflog = result.HeadSHA != "" && result.ReflogHeadSHA != "" && result.HeadSHA != result.ReflogHeadSHA
	return result, nil
}
...
func runIntegrityGit(worktreePath string, args ...string) (string, error) {
	ctx, cancel := context.WithTimeout(context.Background(), worktreeIntegrityTimeout)
	defer cancel()
