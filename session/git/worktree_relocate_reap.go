...
//     this reap is best-effort and returns no error: reapWorktreeWriters already
//     degrades to a no-op on an unreadable process table, and a writer that
//     survives SIGKILL is left for the residue report below rather than turning
//     a working archive into a hard failure.
//
...
func reapRelocationSourceWriters(src, dest string) string {
	if src == "" || src == dest {
		return ""
	}
...
// leftover-source warning above it makes, with the same by-hand suggestion.
func reportRelocationResidue(vacated string) {
	if vacated == "" || !pathExists(vacated) {
		return
	}
	log.WarningLog.Printf(
		"worktree was relocated away from %s, but that pathname exists again: a writer that outlived the "+
			"pre-move reap re-created it by absolute path. The relocated worktree is valid and complete — this "+
			"leftover is not part of it and is not registered with git. Inspect it and remove it by hand with "+
			"`%s`, because a later session deriving the same path would inherit it.",
