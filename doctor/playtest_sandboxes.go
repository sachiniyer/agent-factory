...
// Injectable so the doctor suite never observes the host's Docker containers.
var playtestDockerOutput = func(ctx context.Context, args ...string) ([]byte, error) {
...
// reaping, including its fix-time identity checks; doctor never removes Docker
// containers, even with --fix.
func checkStrandedPlaytestSandboxes(ctx *scanContext, report *Report) {
	const check = "stranded-playtest-sandbox"
	deadline, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	out, err := playtestDockerOutput(deadline, "ps", "-aq", "--filter", "label=af.harness=testbox", "--filter", "name=af-playtest-")
	if errors.Is(err, exec.ErrNotFound) {
		report.Pass(sectionProcesses, check, "Docker is not installed; sandbox inspection is not applicable")
		return
	}
	incomplete := func(detail string) {
		report.markIncomplete(check)
		report.Warn(sectionProcesses, check, detail, "check Docker connectivity, then rerun `af doctor`", false)
	}
	if err != nil {
	...
	out, err = playtestDockerOutput(deadline, append([]string{"inspect"}, ids...)...)
	if err != nil {
	...
	if err := json.Unmarshal(out, &containers); err != nil {
		incomplete("could not decode Docker sandbox inspection")
		return
	}
	if len(containers) != len(ids) {
		incomplete("Docker inspection did not return every listed sandbox")
		return
	}
	found := 0
	complete := true
	for _, c := range containers {
		age, limit, err := c.stranded(ctx.snapAt)
		if err != nil {
			complete = false
