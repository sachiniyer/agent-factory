...
func TestCaptureUpgradePlan_PopulatesMetadataManifest(t *testing.T) {
	...
	plan, err := captureUpgradePlan(lifecycle, candidate, "1.0.200", "", false)
	require.NoError(t, err)
...
func TestTriggerUpgradeActivation_AuthorizesThenQuiescesAndExits(t *testing.T) {
	...
	require.NoError(t, triggerUpgradeActivation(context.Background(), lifecycle, requestExit, candidate, "1.0.200", "", false))
...
func TestTriggerUpgradeActivation_NotReadyKeepsServing(t *testing.T) {
	...
	err := triggerUpgradeActivation(context.Background(), lifecycle, requestExit, candidate, "1.0.200", "", false)
	require.Error(t, err)
...
func TestTriggerUpgradeActivation_AbortsPreparedOnInstallFailure(t *testing.T) {
	...
	exited := false
	err := triggerUpgradeActivation(context.Background(), lifecycle, func() { exited = true }, candidate, "1.0.200", "", false)
	require.Error(t, err)
...
func TestUpgradeTrigger_CapturedJournalDrivesForwardCommitAndRollback(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			plan, err := captureUpgradePlan(lifecycle, candidate, tc.toVersion, "", false)
			require.NoError(t, err)
