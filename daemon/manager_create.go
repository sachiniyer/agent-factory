...
func (m *Manager) CreateSession(ctx context.Context, req CreateSessionRequest) (_ session.InstanceData, retErr error) {
	...
	// A routed pick reserved its account under m.mu so the next create's load
	// snapshot already counts it. The claim lives until the pending projection
	// below takes over the count — any earlier exit must release it, or the
	// account stays charged for a session that never launches.
	defer func() {
		if req.routedAccountClaim != "" {
			m.releaseCreateAccountClaim(req.routedAccountClaim)
		}
	}()
	...
	if req.routedAccountClaim != "" {
		// The row now carries the routed account for every later load snapshot,
		// so the router's claim retires inside this same critical section — no
		// window where the assignment reads as uncounted or double-counted.
		m.releaseCreateAccountClaimLocked(req.routedAccountClaim)
		req.routedAccountClaim = ""
	}
	m.mu.Unlock()
