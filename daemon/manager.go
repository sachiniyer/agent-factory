...
type Manager struct {
	...
	// manager.go's deletedRootRepos, carved out of #2480 pending #2216). The
	// network listener keys used to read it too, until #2480 PR2 made them
	// applied-live (livePosture per request; listen_addr/preview_listen_addr rebind
	// in place), and so did branch_prefix until #4539 resolved it per create.
	// Everything hot-reloadable reads live via Config() instead.
	cfg *config.Config
	...
	// a sibling healing in the same pass resets that clock to now, and
	// without per-project spacing the second strike could land one poll tick
	// later (#3299 review round 9).
	rootHealAbsenceLastStrike map[string]time.Time
	...
	// outlived its caller deadline. The persisted record re-arms the exact identity
...
func newManagerShellWithOptions(cfg *config.Config, transactionID string, opts managerOptions) (*Manager, error) {
	...
	mgr.configAssistants = newConfigAssistantHub(mgr)
	return mgr, nil
}
