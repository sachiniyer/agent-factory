...
func TestValidateTitleRejectsRemoteHookWithoutASCIIAlphanumeric(t *testing.T) {
	...
	for _, title := range []string{
		...
		manager.mu.Lock()
		err := manager.validateTitleAvailableLocked(manager.globalBranchNaming(), repoID, repoPath, title, "claude", runtimeNamespaceRemoteHook, false, nil, false)
		manager.mu.Unlock()
		if err == nil {
	...
	manager.mu.Lock()
	err := manager.validateTitleAvailableLocked(manager.globalBranchNaming(), repoID, repoPath, "日本語", "claude", runtimeNamespaceSandbox, false, nil, false)
	manager.mu.Unlock()
	if err != nil {
...
func TestNextAvailableTitleRejectsGenericRemoteHookSlug(t *testing.T) {
	...
	manager.mu.Lock()
	got, err := manager.nextAvailableTitleLocked(manager.globalBranchNaming(), repoID, repoPath, "日本語", "claude", runtimeNamespaceRemoteHook, nil, false)
	manager.mu.Unlock()
	if err == nil {
