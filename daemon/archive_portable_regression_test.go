...
func TestArchivePortableSuffixSnapshot(t *testing.T) {
	...
	defer restore()
	m.mu.Lock()
	title, err := m.uniqueArchivedTitleLocked(m.globalBranchNaming(), repoID, repoPath, "foo", "claude", runtimeNamespaceLocalTmux, nil)
	m.mu.Unlock()
	require.NoError(t, err)
...
func TestArchivePortableSnapshotReadErrorIsFatal(t *testing.T) {
	...
	defer restore()
	m.mu.Lock()
	_, err = m.uniqueArchivedTitleLocked(m.globalBranchNaming(), repoID, repoPath, "foo", "claude", runtimeNamespaceLocalTmux, nil)
	m.mu.Unlock()
