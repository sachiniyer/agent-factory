...
func (m *Manager) observeTaskRunWhilePaused(repoID, key string, instance *session.Instance) {
	...
	beforeAccountEvidence := instance.AccountLimitObservations()
	...
	if obs.Baseline || obs.Updated || obs.HasPrompt {
		...
		m.persistPollChangeWithIdleEvidence(repoID, instance, before, beforeReset, beforeObserved, beforeAccountEvidence, projectionChanged, churnCheckpoint)
		return
	}
...
func (m *Manager) refreshInstanceStatus(repoID string, instance *session.Instance) {
	...
	manualSwap, _ := instance.PendingManualAccountSwap()
	if _, _, pending := instance.PendingAccountSwap(); pending && (manualSwap || instance.GetLiveness() == session.LiveLimitReached) {
		...
		// normal status probe must not rewrite its checkpointed liveness and route
		// it through ordinary recovery. Manual checkpoints can also be healthy.
		m.clearRemoteLoss(key)
		return
	}
	if !instance.Started() {
	...
	m.persistPollChangeWithIdleEvidence(repoID, instance, before, beforeReset, beforeObserved, beforeAccountEvidence, projectionChanged, settlementCheckpoint)
