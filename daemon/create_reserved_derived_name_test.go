...
func TestValidateTitleRefusesReservedDerivedName(t *testing.T) {
	...
	for _, title := range []string{"ro ot", "r o o t", "  ro ot  "} {
		t.Run(title, func(t *testing.T) {
			...
			err := m.validateTitleAvailableLocked(m.globalBranchNaming(), repoID, repoPath, title, "claude", runtimeNamespaceLocalTmux, false, nil, false)
			if err == nil {
...
func TestReservedNameRuleLeavesTheEnsureLoopAlone(t *testing.T) {
	...
	m := newTitleAdmissionManager()
	if err := m.validateTitleAvailableLocked(m.globalBranchNaming(), repoID, repoPath, session.RootSessionTitle, "claude", runtimeNamespaceLocalTmux, true, nil, false); err != nil {
	...
	if err := m.validateTitleAvailableLocked(m.globalBranchNaming(), repoID, repoPath, "ro-ot", "claude", runtimeNamespaceLocalTmux, false, nil, false); err != nil {
...
func TestValidateTitleRefusesWhitespaceDerivedCollisionWithLiveSession(t *testing.T) {
	...
	err := m.validateTitleAvailableLocked(m.globalBranchNaming(), repoID, repoPath, candidate, "claude", runtimeNamespaceLocalTmux, false, nil, false)
	if err == nil {
