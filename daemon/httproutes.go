...
type HTTPRoute struct {
	...
	//     convenience.
	//
	//   - SuggestSessionName — the last one standing, and it fell too. It takes no
	//     arguments and returns a random unused name, which looks like the one
	//     harmless thing on the surface. But it avoids every live title ACROSS ALL
	//     REPOS (its handler says so), the wordlist is finite, and the sandbox holds
	//     that wordlist in the `af` binary af itself put there — so sampling the
	//     route until the free combinations run out reveals which ones are
	//     persistently missing, i.e. the operator's live autogenerated session
	//     titles and their activity. An oracle can be a route that returns nothing
	//     about its own answer.
	//
	// SO THE TABLE IS EMPTY, and that is the finding rather than an oversight.
	//
	// Every route worth giving a sandbox is parameterised by a host path or a
	// session id, so what it needs is "the caller's OWN repo/session" — which a
	// route-level flag cannot express. And the routes that take no parameter at all
	// turn out to answer questions about global state, because that is the only
	// state they have to answer from. Both halves of the surface fail, for opposite
	// reasons, and no redrawing of this list fixes either.
	//
	// The credential must be bound to its owning session, and the routes must
	// enforce against that owner, BEFORE any entry belongs here. Adding one before
	// then produces a boundary that reads as enforced and is not. Tracked on #2999.
	sandboxAllowed bool
