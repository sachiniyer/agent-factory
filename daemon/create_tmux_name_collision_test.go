...
func TestValidateTitleRejectsTmuxNameCollisions(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			err := m.validateTitleAvailableLocked(m.globalBranchNaming(), repoID, repoPath, candidate, "claude", runtimeNamespaceLocalTmux, false, disk, false)
			if err == nil {
...
func TestTmuxNameCollisionIsScopedToTwoLocalRuntimes(t *testing.T) {
	...
	if err := m.validateTitleAvailableLocked(m.globalBranchNaming(), repoID, repoPath, candidate, "claude", runtimeNamespaceSandbox, false, localRow, false); err != nil {
	...
	if err := m.validateTitleAvailableLocked(m.globalBranchNaming(), repoID, repoPath, candidate, "claude", runtimeNamespaceLocalTmux, false, remoteRow, false); err != nil {
