...
func TestHookNameNamespaceIsGlobalAcrossRepos(t *testing.T) {
	...
	t.Run("concurrent create in another repo is refused", func(t *testing.T) {
		...
		err := manager.validateTitleAvailableLocked(manager.globalBranchNaming(), repoB.ID, repoB.Root, newTitle, "claude", runtimeNamespaceRemoteHook, false, nil, false)
	...
	t.Run("a free hook name is still allowed", func(t *testing.T) {
		manager.mu.Lock()
		err := manager.validateTitleAvailableLocked(manager.globalBranchNaming(), repoB.ID, repoB.Root, "totally-unused", "claude", runtimeNamespaceRemoteHook, false, nil, false)
		manager.mu.Unlock()
		if err != nil {
...
func TestArchivedHookRenameChecksGlobalSlug(t *testing.T) {
	...
	manager.mu.Lock()
	got, err := manager.uniqueArchivedTitleLocked(manager.globalBranchNaming(), repoB.ID, repoB.Root, base, "claude", runtimeNamespaceRemoteHook, nil)
	manager.mu.Unlock()
	if err != nil {
	...
	manager.mu.Lock()
	local, err := manager.uniqueArchivedTitleLocked(manager.globalBranchNaming(), repoB.ID, repoB.Root, base, "claude", runtimeNamespaceLocalTmux, nil)
	manager.mu.Unlock()
	if err != nil {
...
func TestNextAvailableTitlePropagatesFatalHookCheckError(t *testing.T) {
	...
	manager.mu.Lock()
	got, err := manager.nextAvailableTitleLocked(manager.globalBranchNaming(), repoB.ID, repoB.Root, "base", "claude", runtimeNamespaceRemoteHook, nil, false)
	manager.mu.Unlock()
