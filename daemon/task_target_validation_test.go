...
func TestTaskMutations_MissingReservedTargetRequiresMaterialization(t *testing.T) {
	...
	t.Run("unconfigured root", func(t *testing.T) {
		...
		assert.Contains(t, err.Error(), "no root agent is configured", "the refusal names its cause (#3264)")
	...
	t.Run("deleted root", func(t *testing.T) {
		...
		assert.Contains(t, err.Error(), "root")
		assert.Contains(t, err.Error(), "was deleted this daemon run", "the refusal names its cause (#3264)")
