...
func TestNextAvailableTitle_SkipsBranchHeldByArchivedWorktree(t *testing.T) {
	...
	manager.mu.Lock()
	title, err := manager.nextAvailableTitleLocked(manager.globalBranchNaming(), repoID, repoPath, "sweep", "claude", runtimeNamespaceLocalTmux, nil, false)
	manager.mu.Unlock()
...
func TestNextAvailableTitle_LongTitleWalkConverges(t *testing.T) {
	...
	manager.mu.Lock()
	title, err := manager.nextAvailableTitleLocked(manager.globalBranchNaming(), repoID, repoPath, base, "claude", runtimeNamespaceLocalTmux, nil, false)
	manager.mu.Unlock()
...
func TestNextAvailableTitle_CeilingFailureNamesHeldSuffixHolders(t *testing.T) {
	...
	manager.mu.Lock()
	_, err := manager.nextAvailableTitleLocked(manager.globalBranchNaming(), repoID, repoPath, "sweep", "claude", runtimeNamespaceLocalTmux, nil, false)
	manager.mu.Unlock()
	require.Error(t, err)
